...
class DashboardsApi(object):
    ...
    def execute_dashboard(self, dashboard_execute_request, **kwargs):  # noqa: E501
        ...
        return self.execute_dashboard_with_http_info(dashboard_execute_request, **kwargs)  # noqa: E501

    def execute_dashboard_with_http_info(self, dashboard_execute_request, **kwargs):  # noqa: E501
        """Execute a dashboard  # noqa: E501

        Execute every module in a dashboard with shared time-range and filter overrides, returning the columns and rows produced by each module.  Modules execute in parallel and are reported in the dashboard's stored module order. Per-module failures (parse, dispatch) land in the `error` field of that module's result; siblings continue to run.  Known v1 limitations: - Template variables (`{var_name}` in stored OQL) are not substituted   server-side. Modules containing unsubstituted placeholders will   fail at parse time.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.execute_dashboard_with_http_info(dashboard_execute_request, async_req=True)
        >>> result = thread.get()
        ...
        :param _request_timeout: timeout setting for this request. If one
                                 ...
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        :return: tuple(list[DashboardExecuteResult], status_code(int), headers(HTTPHeaderDict))
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        for key, val in six.iteritems(local_var_params['kwargs']):
            if key not in all_params:
                raise ApiTypeError(
                    ...
                    " to method execute_dashboard" % key
                )
            local_var_params[key] = val
        del local_var_params['kwargs']
        # verify the required parameter 'dashboard_execute_request' is set
        if self.api_client.client_side_validation and ('dashboard_execute_request' not in local_var_params or  # noqa: E501
                                                        local_var_params['dashboard_execute_request'] is None):  # noqa: E501
            raise ApiValueError("Missing the required parameter `dashboard_execute_request` when calling `execute_dashboard`")  # noqa: E501
        ...
        body_params = None
        if 'dashboard_execute_request' in local_var_params:
            body_params = local_var_params['dashboard_execute_request']
        # HTTP header `Accept`
        ...
        return self.api_client.call_api(
            ...
            response_type='list[DashboardExecuteResult]',  # noqa: E501
